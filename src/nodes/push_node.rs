...
impl<'node> PushNode<'node> {
    ...
    pub fn new(server : &server::Server, sr: u32, channels: u32) -> Result<PushNode> {
        ...
        Ok(PushNode { handle : node_handle, low_callback: None, underrun_callback: None })
    }
    ...
    pub fn set_low_callback<F>(&mut self, callback: F) -> Result<()>
     where F: 'node + FnMut(&mut PushNode) {
        ...
        check(unsafe { libaudioverse_sys::Lav_pushNodeSetLowCallback(self.handle, Some(callback_handler), cb_ptr as *mut _) })?;
        Ok(())
    }
    ...
    pub fn set_underrun_callback<F>(&mut self, callback: F) -> Result<()>
     where F: 'node + FnMut(&mut PushNode) {
        ...
        check(unsafe { libaudioverse_sys::Lav_pushNodeSetUnderrunCallback(self.handle, Some(callback_handler), cb_ptr as *mut _) })?;
        Ok(())
    }
...
extern "C" fn callback_handler(node_handle: libaudioverse_sys::LavHandle, userdata: *mut c_void){ 
    ...
    let mut node = PushNode { handle: node_handle, low_callback: None, underrun_callback: None };
    closure(&mut node)
}